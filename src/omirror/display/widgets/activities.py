...
from omirror.widgets import activities as data


class ActivitiesWidget(ListWidget):
    def get_info(self) -> None:
        self.header = text_surface("Activities", "Thin", 32, 255)
        self.items = []
        for i in range(LIST_ITEM_COUNT):
            if len(data.sorted_activities) > i:
                act = data.sorted_activities[i]
                t = act["text"]
                self.items.append(
                    (
                        text_surface(
                            self._format_date(act["date"]), "Thin", 16, LIST_ITEM_ALPHAS[i] - 15
                        ),
                        text_surface(
